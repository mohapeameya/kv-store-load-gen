...
pthread_mutex_t conn_mutex, req_mutex;
...
void run(int time_in_sec, int no_workers, ofstream& logFile){
...
int main(int argc, char *argv[])
{
    ...
    int time_in_sec;
    if(argc!=6){
        printf("Usage: %s <server_ip> <server_port> <min_threads> <max_threads> <time_per_thread_in_secs>\n",argv[0]);
        printf("Test time will be equal to (<max_threads> - <min_threads> + 1)*<time_per_thread_in_secs> seconds\n");
        printf("Ex. %s 127.0.0.1 55555 5 10 2\n", argv[0]);
        return -1;
    }
    strcpy(serv_ip, argv[1]);

    if(atoi(argv[2]) <= 0)/*atoi function does not detect errors, use strtol(param[1], NULL, 10) instead*/{
        printf("<server_port> must be an integer\n");
        return -1;
    } else {
    ...
    if(atoi(argv[5]) <= 0){
        /*atoi function does not detect errors, use strtol(param[1], NULL, 10) instead*/
        printf("<time_per_thread_in_secs> must be a positive integer\n");
        return -1;
    } else {
    ...
    while(no_workers <= max_threads){
        run(time_in_sec, no_workers, logFile);
        no_workers++;
    }
